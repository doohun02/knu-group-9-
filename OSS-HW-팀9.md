...
파이썬은 한 줄씩 읽고 실행하는 언어.

컴파일러 언어(C, C++)는 먼저 전체 코드를 컴파일한 후 실행하지만,
파이썬은 코드를 위에서 아래로 즉시 해석하고 실행할 수 있다

2. 문법이 간결하고 직관적
파이썬은 사람이 읽기 쉬운 문법을 목표로 만들어짐.

4. 동적 타이핑 (Dynamic Typing)
변수 선언할 때 자료형을 명시하지 않아도 됨

파이썬이 실행 중에 타입을 자동으로 판단함.

4. 객체 지향 언어 (Object-Oriented Programming, OOP)
파이썬에서는 **모든 것이 객체(Object)**.

숫자도, 문자열도, 함수도, 클래스도 객체.

객체 지향 프로그래밍을 지원해서 클래스, 상속, 다형성 등을 사용할 수 있음.

예를 들어, Java로 Hello World를 출력하려면 여러 줄이 필요하지만,
파이썬은 단 한 줄이면 됨.
# (3) 파이썬 활용예시 - 허준혁
1.파이게임
