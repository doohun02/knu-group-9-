...
먼저 우리는 .py 파일에 파이썬 코드를 작성한다. 이 코드는 사람이 이해하기 쉬운 고수준 언어로 되어 있다. 작성이 끝나고 코드를 실행하면, 파이썬 인터프리터가 코드를 한 줄씩 읽기 시작한다. 이때 문법에 오류가 있는지 먼저 검사하고, 문제가 없다면 코드를 내부적으로 분석하고 해석하는 과정을 거친다. 이 과정을 **파싱(parsing)**이라고 한다.
그다음 파이썬은 코드를 **바이트코드(bytecode)**라는 중간 단계의 형태로 변환한다. 바이트코드는 사람이 읽을 수는 없지만, 컴퓨터가 빠르게 처리할 수 있도록 만들어진 코드이다. 이렇게 변환된 바이트코드는 **PVM(Python Virtual Machine, 파이썬 가상 머신)**이라는 프로그램이 실행하게 된다. PVM은 바이트코드를 받아 실제 컴퓨터가 이해할 수 있는 동작으로 바꾸고 실행한다. 우리가 터미널이나 콘솔에서 보는 실행 결과는 바로 이 단계에서 출력된다.
...
요약하면, 파이썬은 우리가 작성한 코드를 읽고, 바이트코드로 바꾼 뒤, 가상 머신이 실행하여 결과를 출력하는 방식으로 작동한다. 이 덕분에 파이썬은 사용하기 쉽고, 다양한 분야에서 폭넓게 사용되는 언어가 되었다.
